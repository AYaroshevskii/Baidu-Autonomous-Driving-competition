...
class RandomBrightnessContrast:
    def __init__(self, limit_alpha=0.3, limit_beta=0.3, prob=0.1):
        self.limit_alpha = limit_alpha
        self.limit_beta = limit_beta
        self.prob = prob

    def apply(self, img, mask, regr, alpha=1.0, beta=0.0):
        if random.random() < self.prob:
...
train_transforms = DualCompose(
    [RandomBrightnessContrast(prob=0.35), RandomGamma(prob=0.35), RBlur(prob=0.2)]
)
