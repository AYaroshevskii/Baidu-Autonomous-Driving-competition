...
scheduler_decay = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, 
                  mode='min',
                  factor = 1/np.sqrt(10),
                  threshold=0, 
                  patience = 1, 
...
for epoch in range(start_epoch, NUM_EPOCHS):
    ...
    for (img_batch, mask_batch, regr_batch) in tqdm(data.DataLoader(train_dataset, batch_size = BATCH_SIZE, shuffle = True,
                                                num_workers=2, worker_init_fn=_worker_init_fn), disable=True):
    ...
    with torch.no_grad():
      ...
      for (img_batch, mask_batch, regr_batch) in tqdm(data.DataLoader(valid_dataset, batch_size = BATCH_SIZE, shuffle = False,
                                                num_workers=2, worker_init_fn=_worker_init_fn), disable=True):
          ...
          for s in range(y_pred.shape[0]):
            mean_ap_metric += average_precision_score(mask_batch[s,:,:].cpu().data.numpy().astype(int).reshape(-1),
                                                      mask_pred[s,:,:].cpu().data.numpy().astype(int).reshape(-1))
            mean_f_score += f1_score(mask_batch[s,:,:].cpu().data.numpy().astype(int).reshape(-1),
                                     mask_pred[s,:,:].cpu().data.numpy().astype(int).reshape(-1))
      ...
      if np.mean(regr_val_score) < best_score:
        save_checkpoint("models/{}_fold{}.pth".format(enc_name, GLOBAL_FOLD), model, optimizer)
        best_score = np.mean(regr_val_score)
       
      print ("Val regr Loss : {}".format(np.mean(regr_val_score)) )
      print ("Regr Loss : {}".format(np.mean(train_regr_loss)))
      print ("Train Loss : {}, Validation Loss : {}".format(np.mean(train_loss) , val_loss))
